...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await launchCampaignTree({
      ...
      campaignBudgetOptimization: Boolean(body.campaignBudgetOptimization),
      ...
      adSets: body.adSets.map((as: Record<string, unknown>) => ({
        ...
        startTime: typeof as.startTime === "string" && as.startTime ? as.startTime : undefined,
        endTime: typeof as.endTime === "string" && as.endTime ? as.endTime : undefined,
        ads: (as.ads as Record<string, unknown>[]).map((ad) => ({
          ...
          cta: ad.cta,
          format: ad.format === "carousel" ? "carousel" : "auto",
