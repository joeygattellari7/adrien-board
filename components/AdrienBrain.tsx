...
function ProposalCard({ proposal, onScheduled }: { proposal: ContentProposal; onScheduled: (platform: string) => void }) {
  ...
  async function approve() {
    ...
    setError(null);
    try {
      const mediaBase64 = proposal.asset?.base64 ?? generatedBase64 ?? undefined;
      const res = await fetch("/api/content/schedule", {
        ...
        body: JSON.stringify({
          ...
          mediaBase64,
          mediaType: mediaBase64 ? "image" : undefined,
  ...
  const previewImage = proposal.asset?.base64 ?? generatedBase64;
  ...
  async function addToLibrary(file: File) {
    ...
    try {
      ...
      await fetch("/api/content/library", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ base64, label: file.name }),
      });
      await loadLibrary();
    } finally {
      setUploading(false);
          ...
          <div className="lg:col-span-2 rounded-xl border border-white/10 bg-zinc-900 p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <label className="text-xs font-medium rounded-lg border border-violet-500/40 text-violet-400 px-3 py-1.5 hover:bg-violet-950/30 cursor-pointer">
                ...
                <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && addToLibrary(e.target.files[0])} />
              </label>
            </div>
            <p className="text-[11px] text-white/40 mb-3">Reels/static shots to draw from before generating anything new. Video isn't stored here yet — same limitation as Content Studio.</p>
